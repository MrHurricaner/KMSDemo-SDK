...
import com.juzix.sdk.utils.JsonMapUtil;

import java.util.HashMap;
import java.util.Map;

public class MpcSdkTestV2 {
     ...
     */
    private static void testECDSAKeyGen() throws Exception {
        //声明计算参与方
        MpcSdk p1 = MpcSdk.getMpcSdk(ComputeEcdsaKeyGen.PARTY_P1);

        //step=0  生成计算的会话
        MpcContext mpcContext = p1.createSession(ComputeEcdsaKeyGen.COMPUTE_KEY);
        ...
        //step=2 P2 计算
        String result = testECDSAKeyGenDone(JsonMapUtil.toJson(mpcContext.getToOthers()));

        //step=3 P1计算
        mpcContext = p1.nextStep(JsonMapUtil.toMap(result));
    ...
    private static String testECDSAKeyGenDone(String getToOthers) throws Exception {
        ...
        Map<String, Map<String, Object>> result = JsonMapUtil.toMap(getToOthers);
        MpcContext mpcContext = p2.nextStep(result, p2_input);
        ...
        return JsonMapUtil.toJson(mpcContext.getToOthers());
    }
     ...
     * SIGN_S（签名的S值）
     */
    private static void testECDSASign() throws Exception {
        //声明计算参与方
        MpcSdk p1 = MpcSdk.getMpcSdk(ComputeEcdsaSign.PARTY_P1);


        //生成计算的会话 step=0
        MpcContext mpcContext = p1.createSession(ComputeEcdsaSign.COMPUTE_KEY);
        ...
        //P2 计算 step=2
        String result = testECDSASignSend(JsonMapUtil.toJson(mpcContext.getToOthers()));

        //P1计算 step=3
        mpcContext = p1.nextStep(JsonMapUtil.toMap(result));

        //P2计算 step=4
        result = testECDSASignMult(JsonMapUtil.toJson(mpcContext.getToOthers()));
        ...
        //P2计算 step=6
        testECDSASignP2VerifySign(JsonMapUtil.toJson(mpcContext.getToOthers()));
    }

    private static String testECDSASignSend(String getToOthers) throws Exception {
        //P2 计算 step=2
    ...
    private static String testECDSASignMult(String getToOthers) throws Exception {
        //P2 计算 step=4
        MpcSdk p2 = MpcSdk.getMpcSdk(ComputeEcdsaSign.PARTY_P2);
        Map<String, Map<String, Object>> result = JsonMapUtil.toMap(getToOthers);
        MpcContext mpcContext = p2.nextStep(result);

        return JsonMapUtil.toJson(mpcContext.getToOthers());
    }

    private static void testECDSASignP2VerifySign(String getToOthers) throws Exception {
        //P2 计算 step=6
        MpcSdk p2 = MpcSdk.getMpcSdk(ComputeEcdsaSign.PARTY_P2);
        Map<String, Map<String, Object>> result = JsonMapUtil.toMap(getToOthers);
        MpcContext mpcContext = p2.nextStep(result);
